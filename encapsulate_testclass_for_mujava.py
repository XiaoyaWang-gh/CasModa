"""
File: 
    encapsulate_testclass_for_mujava.py
Description: 
    修改自encapsulate_into_a_test_class.py
...
Usage: 
    直接在图形界面运行
Input Files: 
    生成的测试方法文件(mujava/attempt-x/prefixes.txt)，
...
Output Files: 
    某个pro的全部测试类文件(total_x_attempts_test_prefixes)
"""
...
def main():
    ...
    test_methods_suffix = current_path / \
        f"txt_repo/validation/{project_name[:-5]}/output/mujava"
    ...
    # 确定输出路径
    tmp_dir_name = f"total_{total_attempt_num}_attempts_test_prefixes"
    ...
    for attempt_id in range(1, total_attempt_num+1):
        ...
        test_methods_file = test_methods_suffix / attempt_dir / "prefixes.txt"

        with open(test_methods_file, 'r') as f:
            original_lines = f.readlines()
        for line in original_lines:
            print(f"📢第{attempt_id}批-读入的prefixes单行内容是：\n{line}")
