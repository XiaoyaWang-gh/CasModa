"""
File:langchain4j-CUTE_pipeline.py
Description:从CUTE_pipeline.py复制过来，做一切修改以适配langchain4j的four projects
Created Date:24.03.18
Last Updated Date:24.03.18
Usage: click 'run' in UI
Input Files: global pro
Input Parameters: langchain4j-prefix/query_set, langchain4j-oracle/query_set, prefix/demo_pool, oracle/demo_pool
Output Files:langchain4j-validation/pro/output/
Output Results:
"""
import os
import ast
...
message_list = [
    {"role": "system", "content": "You are a proficient and helpful assistant in java testing with JUnit5 and assertJ."}
]


def call_chatgpt_wo_history(prompt_str:str):

    # 提问环节
    one_question = {"role": "user", "content": prompt_str}
    query_list = message_list.copy()
    query_list.append(one_question)
    answer = generate_chat_completion(query_list)
    # 返回结果
    return answer
...
# 给路径占位符赋值
pro = "openai" # should be one of [openai,qianfan,zhipu,core]
sample_order = "random"
shot_num = 5

# 准备数据路径
prefix_queryset_dir_path = f"txt_repo/langchain4j-prefix/query_set/{pro}"
prefix_demopool_dir_path = f"txt_repo/prefix/demo_pool/"
oracle_demopool_dir_path = f"txt_repo/oracle/demo_pool/"

testClass_place_path = f"txt_repo/langchain4j-validation/{pro}/testClass_place.json"
testClass_dir_path = f"txt_repo/langchain4j-validation/{pro}/test_class_content"
common_project_to_path = "txt_repo/langchain4j-validation/common/pro_with_ver_to_path.json"

testunit_compile_stderr_dir_path = f"txt_repo/langchain4j-validation/{pro}/output/testunit_compile"
testunit_execute_stdout_dir_path = f"txt_repo/langchain4j-validation/{pro}/output/testunit_execute"


overall_result_file = f"txt_repo/langchain4j-validation/{pro}/output/overall_result.txt"
passed_unit_file = f"txt_repo/langchain4j-validation/{pro}/output/passed_test_unit.txt"
check_chatgpt_file = f"txt_repo/langchain4j-validation/{pro}/output/check_chatgpt.txt"
original_chatgpt_out_file = f"txt_repo/langchain4j-validation/{pro}/output/original_chatgpt_out.txt"
check_prompt_file = f"txt_repo/langchain4j-validation/{pro}/output/check_prompt.txt"
...
def compile_mytest(proName: str, test: str, id: int, err_save_path: str) -> tuple[bool, str]:
    ...
    focal_class_path, test_class_path, middle_path, _ = find_objects_by_pro_with_ver(
    ...
    write_to_file(test_class_save_path+"/" + test_class_save_name, final_test_class, "w")
    # 编译
...
def execute_mytest(proName: str, test: str, id: int, out_save_path: str) -> tuple[bool, str]:
    ...
    with open(common_project_to_path, "r") as f:
        common_info = json.load(f)
    common_info_json_list = [dict(item) for item in common_info]
    focal_class_path, test_class_path, middle_path, junit5_exe_cp = find_objects_by_pro_with_ver(
        proName, pro_with_ver, common_info_json_list)
    test_class_save_path = os.path.join(
        test_class_path, middle_path, "generated_by_chatgpt")
    os.makedirs(test_class_save_path, exist_ok=True)
    ...
    if compile_result.returncode != 0:
        ...
        return False, compile_result.stderr  # 如果编译失败，说明不是第一次进入执行
    # 执行测试类
    execute_command = r'java -jar "C:\Libraries\maven-3.5.0\lib\junit-platform-console-standalone-1.10.0.jar" --class-path '  + \
        '\"' + junit5_exe_cp + '\" --select-class ' + \
        middle_path.replace('\\','.') + ".generated_by_chatgpt."+test_class_name+str(id)
...
def repair_bf(test_with_only_prefix: str, compile_out: str, query: Prefix_datapoint, chatgpt_max_num: int) -> str:
    ...
    for i in range(1, chatgpt_max_num, 1):  # retry外包状态码的问题，这里解决空值和不含代码的问题
        ...
        chatgpt_response = call_chatgpt_wo_history(bf_repair_prompt)
        # time.sleep(6)
...
def repair_tf_test_unit(test_unit: str, execute_out: str, query: Prefix_datapoint, chatgpt_max_num: int) -> str:
    ...
    for i in range(1, chatgpt_max_num, 1):  # retry外包状态码的问题，这里解决空值和不含代码的问题
        ...
        chatgpt_response = call_chatgpt_wo_history(tf_repair_prompt)
        # time.sleep(6)
        if hascode(chatgpt_response):  # 如果调用返回中包含代码块(而不仅仅是抱歉)，就跳出循环
            has_code_flg = True
            write_to_file(original_chatgpt_out_file,
...
def main():
    ...
    load_dotenv()
    # 读取环境变量
    querynum_dict_str = os.getenv("LC4J_QUERYNUM_DICT")
    ...
    old_pro_list_str = os.getenv("PRO_SET")
    old_pro_list = ast.literal_eval(old_pro_list_str)

    # sample_idx_lst_dict_str = os.getenv("NEW_SAMPLE_IDX_LST_DICT")
    # print(f"sample_idx_lst_dict_str\n : {sample_idx_lst_dict_str}")
    ...
    # 这里替换成 prefix_demopool_list
    prefix_demopool_list = []
    for p in old_pro_list:
        prefix_demopool = Prefix_Dataset(
            prefix_demopool_dir_path+p,
            "constr_sign.txt",
            "test_input.txt",
            "classname.txt",
            "focalname_paralist.txt",
        ...
        prefix_demopool_list.append(prefix_demopool)
    ...
    # 进入循环啦
    start_idx = 16  # 1
    end_idx = 16  # len(prefix_queryset_list)
    aborted_num = 0
    ...
    for id in range(start_idx, end_idx+1, 1):
        try:
            ...
            # 这里替换成 prefix_demopool_list
            now_prefix_demopool_list = []
            for prefix_demopool in prefix_demopool_list:
            ...
            write_to_file(check_prompt_file,'prefix_gen_prompt:\n'+prefix_gen_prompt+'\n','a')
            logger.info('-'*100)
            # 生成prefix
            return_prefix = ""
            for i in range(1, chatgpt_max_num, 1):  # retry外包状态码的问题，这里解决空值和不含代码的问题
                ...
                return_prefix = call_chatgpt_wo_history(prefix_gen_prompt)
                # time.sleep(6)
            ...
            raw_oracle_demopool_list = []
            for p in old_pro_list:
                oracle_demopool = Oracle_Dataset(
                    oracle_demopool_dir_path+p,
                    "test_method.txt",
                    "focalname_paralist.txt",
                    "test_name.txt",
                    "oracle.txt"
                )
                raw_oracle_demopool_list.append(oracle_demopool)
            oracle_demopool_list = []
            for dp in raw_oracle_demopool_list:
                oracle_demopool_list += dp.demopool_parse()
            ...
            write_to_file(check_prompt_file,'oracle_gen_prompt:\n'+oracle_gen_prompt+'\n','a')
            return_oracle = ""
            for i in range(1, chatgpt_max_num, 1):  # retry外包状态码的问题，这里解决空值和不含代码的问题
                write_to_file(check_chatgpt_file,
                            f"第{i}次调用chatgpt 用于生成oralce\n")
                return_oracle = call_chatgpt_wo_history(oracle_gen_prompt)
                # time.sleep(6)
    ...
    # 打印整个项目的运行结果
    # total_num = aborted_num + bf_num + tf_num + passed_num
